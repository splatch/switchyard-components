...
public class HttpContextMapper extends BaseRegexContextMapper<HttpBindingData> {
     ...
     */
    @Override
    public void mapFrom(HttpBindingData source, Context exchangeContext, Context messageContext) throws Exception {
        if (source instanceof HttpResponseBindingData) {
            HttpResponseBindingData response = (HttpResponseBindingData) source;
            messageContext.setProperty(HttpComposition.HTTP_RESPONSE_STATUS, response.getStatus()).addLabels(HTTP_LABELS);
        } else {
            HttpRequestBindingData request = (HttpRequestBindingData) source;
            if (request.getRequestInfo() != null) {
                messageContext.setProperty(HttpComposition.HTTP_REQUEST_INFO, request.getRequestInfo()).addLabels(HTTP_LABELS);
            }
        }
        ...
        while (entries.hasNext()) {
            ...
            if (matches(name)) {
                ...
                if ((values != null) && (values.size() == 1)) {
                    messageContext.setProperty(name, values.get(0)).addLabels(HTTP_LABELS);
                } else if ((values != null) && (values.size() > 1)) {
                    messageContext.setProperty(name, values).addLabels(HTTP_LABELS);
                }
            }
        }
    }
    ...
    public void mapTo(Context exchangeContext, Context messageContext, HttpBindingData target) throws Exception {
